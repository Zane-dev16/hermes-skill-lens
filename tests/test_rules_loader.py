...
def test_core_pack_loads_offline_with_expected_shape() -> None:
    ...
    assert pack.version == "2026.08.10"  # YYYY.MM.N semver (SPEC §15); patch bump = new rule
    assert len(pack.rules) >= 40  # 51 at 2026.08.10 (D-060: deterministic rules scale on merit)
